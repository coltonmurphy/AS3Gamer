...
//---------------------------- AddSpawnPoint ----------------------------------
//-----------------------------------------------------------------------------
public function AddSpawnPoint(x:Numer, y:Number):void
{
  m_SpawnPoints.push(Vector2D(x,y));
}


//----------------------- AddHealth__Giver ----------------------------------
//-----------------------------------------------------------------------------
public function AddHealthGiver(std::ifstream& in)
{
  var hg:TriggerHealthGiver = new TriggerHealthGiver(in);

  m_TriggerSystem.Register(hg);

  //let the corresponding navgraph node point to this object
  var node = m_pNavGraph.getNode(hg.graphNodeIndex());
...
//----------------------- AddWeapon__Giver ----------------------------------
//-----------------------------------------------------------------------------
public function AddWeaponGiver( type_of_weapon:int, std::ifstream& in)
{
  var wg:TriggerWeaponGiver = new TriggerWeaponGiver(in);
  ...
  //let the corresponding navgraph node point to this object
  var node = _navGraph.getNode(wg.graphNodeIndex());
...
//-----------------------------------------------------------------------------
public function loadMap( filename:String ):Boolean
{  
  ...
  _cellSpaceNeighborhoodRange = CalculateAverageGraphEdgeLength(*m_pNavGraph) + 1;
...
//-----------------------------------------------------------------------------
public function PartitionNavGraph():void
{
  if ( _spacePartition ) 
	   delete _spacePartition;

  var cellsX:Number = 20;
  var cellsY:Number = 20;
  
  //Contains navgraph nodes
  _spacePartition = new CellSpacePartition( _sizeX, _sizeY, cellsX, cellsY, _navGraph.numNodes());

  //add the graph nodes to the space partition
  var NodeItr:NodeIterator = _navGraph;
  for ( var pN=NodeItr.begin();!NodeItr.end();pN=NodeItr.next())
  {
	_spacePartition.addEntity( pN );
  }   
}
...
//-----------------------------------------------------------------------------
public function AddSoundTrigger(pSoundSource:IGameEntity,  range:Number)
{
    _triggerSystem.register(new TriggerSoundNotify( pSoundSource, range ));
}
