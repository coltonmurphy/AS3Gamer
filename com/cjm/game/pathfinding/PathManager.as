...
private:
  ...
  private var  m_SearchRequests:Vector<PathPlanner>;
  ...
  //requests
   private var  m_iNumSearchCyclesPerUpdate:int;

public:
    
  PathManager( NumCyclesPerUpdate:int )
  {
...
//-----------------------------------------------------------------------------
template <class path_planner>
public function UpdateSearches():void
{
...
public function register(pPathPlanner:PathPlanner )
{
  ...
  if( m_SearchRequests.indexOf( pPathPlanner ) < 0 )                 
  { 
    //add to the list
    m_SearchRequests.push(pPathPlanner);
  }
}
...
{
	var i:Number = m_SearchRequests.indexOf( pPathPlanner );
	
	if ( i > -1 )
	{
		m_SearchRequests.splice( i, 1 );
	}
